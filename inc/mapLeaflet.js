function refresh(estimatedvelovNumber, bloqueTF, VAEFlag )
{
	bloqueTF = typeof bloqueTF !== 'undefined' ?  bloqueTF : 0; //si 1 : vélib bloqué (min 2J ou 3J), si 0 velov dispo
	...
	removeMarkersToMap();
	//addMarkersToMap();
	getStations(estimatedvelovNumber, bloqueTF, VAEFlag );
...
function handleLocationError(browserHasGeolocation, infoWindow2, pos) {
	...
	infoWindow2.setContent(browserHasGeolocation ?
						  'Error: The Geolocation service failed. <a href=https://velov.philibert.info/carte-des-stations.php>Did you try https?</a>' :
						  'Error: Your browser doesn\'t support geolocation.');
	infoWindow2.open(map);
  }

function getStations(estimatedvelovNumber,bloqueTF,VAEFlag)
{
   bloqueTF = typeof bloqueTF !== 'undefined' ?  bloqueTF : 0; //si 1 : vélib bloqué (min 2J ou 3J), si 0 velov dispo
   VAEFlag =  typeof VAEFlag !== 'undefined' ?  VAEFlag : 0; //0 = tous, 1 = Meca, 2 = VAE 
   ...
   var xmlhttp;
	...
	xmlhttp.onreadystatechange = function(event) {
		...
		if (this.readyState === XMLHttpRequest.DONE) {
			if (this.status === 200) {
				...
				addMarkersToMap(estimatedvelovNumber, bloqueTF, VAEFlag);
	...
	url='./api/stationList.api.php?v=web&d='+estimatedvelovNumber;
...
function addMarkersToMap(estimatedvelovNumber, bloqueTF, VAEFlag)
{
	...
	{ 
		...
		// détermination de la valeur nbr velov du marker en fonction du mode
		if(estimatedvelovNumber==0) // nombre de velov officiel
		{
			//0 = tous, 1 = Meca, 2 = VAE 
			if(VAEFlag == 0)
		...
		{
			if(bloqueTF ==1)
				nbBikeMarker = Math.max(0,parseInt(locations[i]['stationMinvelovNDay'])).toString();
			else
			{				
				//0 = tous, 1 = Meca, 2 = VAE 
				if(VAEFlag == 0)
					nbBikeMarker = Math.max(0,parseInt(locations[i]['stationNbBike'])+parseInt(locations[i]['stationNbEBike'])-parseInt(locations[i]['stationMinvelovNDay'])).toString();
				else if (VAEFlag ==1)
					nbBikeMarker = Math.max(0,parseInt(locations[i]['stationNbBike'])-parseInt(locations[i]['stationMinvelovNDay'])+parseInt(locations[i]['stationMinEvelovNDay'])).toString();
				else if (VAEFlag ==2)					
					nbBikeMarker = Math.max(0,parseInt(locations[i]['stationNbEBike'])-parseInt(locations[i]['stationMinEvelovNDay'])).toString();
			}
		}
		
		if(bloqueTF ==1) // selection du market pour la carte velov bloqué
		{
			if(nbBikeMarker<1)
			{
		...
		{
			if( locations[i]['stationState']!='OPEN')
			{
		...
		'<br><br> Nombre de velov: ' +locations[i]['stationNbBike'] +' ( et "Park+": ' +locations[i]['stationNbBikeOverflow'] +')';
		...
		if(parseInt(locations[i]['stationMinvelovNDay'])+parseInt(locations[i]['stationvelovMinvelovOverflow']) > 0 )
		{
			if(estimatedvelovNumber == 0)
				estimatedvelovNumberDisplayedIW= 3;
			else	
				estimatedvelovNumberDisplayedIW = estimatedvelovNumber;
			
			infoWindowContent = infoWindowContent +
			'<br> Sur les '+estimatedvelovNumberDisplayedIW+' derniers jours il n\'y a jamais eu moins de ' + locations[i]['stationMinvelovNDay'] + ' velov ( dont ' + locations[i]['stationMinEvelovNDay'] + ' VAE) (et '+locations[i]['stationvelovMinvelovOverflow']+' en park+)' ; 
			
			if( (parseInt(locations[i]['stationMinvelovNDay'])+parseInt(locations[i]['stationvelovMinvelovOverflow'])) == locations[i]['tot_station_nb_bike'] )
			{			
		...
		infoWindowContent = infoWindowContent + '<p>Plus d\'infos: <a href="https://velov.nocle.fr/station.php?code='+locations[i]['stationCode']+'" target="_blank">velov.nocle.fr</a> ';
...
function displayHeatMap()
{
	...
	{	
		if(parseInt(locations[i]['stationvelovExit'])>maxValue)
			maxValue = parseInt(locations[i]['stationvelovExit']);	
	}
	
	
	for(var i = 0; i< locations.length;i++)
	{
		if(parseInt(locations[i]['stationvelovExit'])>0)
		{	
			heatMapData.push([locations[i]['stationLat'], locations[i]['stationLon'],(locations[i]['stationvelovExit'])/maxValue]);
		}
	}	
...
function displayMvtMap()
{
	...
	{ 
		...
		nbBikeMarker = (parseInt(locations[i]['stationvelovExit'])).toString();
		if(nbBikeMarker>400)
			nbBikeMarker = 400;
